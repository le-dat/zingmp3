    ...
    <div className={clsx("scrollbar", style.wrapper)}>
      {lyric?.map((item: any, index: number) => {
        ...
        return (
          <p
            ref={
              words[0].startTime <= currentTime && words[words.length - 1].endTime >= currentTime ? lyricRef : undefined
            }
            className={clsx(style.paragraph, {
              [style.active]: words[0].startTime <= currentTime && words[words.length - 1].endTime >= currentTime,
            })}
            key={index}
          >
              ...
              return (
                <span
                  ...
                  className={clsx(style.word, {
                    [style.active]: wordItem?.startTime <= currentTime && wordItem?.endTime >= currentTime,
                  })}
                >
                  {wordItem?.data}
                </span>
